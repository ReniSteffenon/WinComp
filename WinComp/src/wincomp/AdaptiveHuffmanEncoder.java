...
public class AdaptiveHuffmanEncoder {
    ...
    private FileOutputStream setupOutputFile(String absolouteFileName){
        ...
        String path = "";
        int tamanhoSPath = absolouteFileName.split("\\\\").length;
        for(int i=0; i< tamanhoSPath; i++){
            if(i<absolouteFileName.split("\\\\").length-1){
                path +=absolouteFileName.split("\\\\")[i]+"\\";
            }
        }
        //retrieve the raw file name by removing everything before the absoloute filname
        String filename = absolouteFileName.split("\\\\")[tamanhoSPath-1];
        //The original file name split into the parts before and after the first "." so that the "compressed" file name part
        //can be inserted
        

        //String[] compressedFileNameComps = filename.split(".");
        ...
        String compressedFileName = path + "\\" + filename + ".compressed" + this.numberOfBitsToUse;
